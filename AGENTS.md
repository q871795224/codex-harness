...
- Claude Provider 由 `src-tauri/claude-adapter/daemon.mjs` 常驻进程承载，通过 `~/.codex-harness/claude-provider.sock` 与 Harness 连接；关闭 Harness 不能停止 daemon 或 active turn。`adapter.mjs` 仅保留为实验参考，不得作为生产 runtime 入口。
