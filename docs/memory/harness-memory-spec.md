...
2026-09-21 调整：目录和模板完成初步对齐后，先交付按钮触发的显式记忆链路，再调优提炼质量；完整编辑界面和自动召回后续接入。名称采用唯一 domain name / workspace name，管理表复用 state.sqlite，scope 放入 YAML front matter。记忆作为核心能力，提炼模型、推理强度、提示词、轮数及预算通过“设置 → 记忆”管理。实现范围与验证见 [阶段 1](phase-1/README.md)。
...
| 领域与工作区如何关联? | 在“设置 → 工作区”管理领域并多选 tag；Agent 只选择已关联领域，不创建领域或修改关联 |
