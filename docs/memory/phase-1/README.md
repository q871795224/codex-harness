...
记忆正文以 Markdown 为准，SQLite 先只保存身份与关联，不保存另一份正文。记忆表随 Harness 状态库初始化，点击保存时登记工作区和领域。现有应用工作区的内部标识保持不变，复用其 Git common directory 归属解析结果进行映射。本次开发验证使用临时目录，没有迁移或写入真实记忆。
...
6. Harness 校验整批候选的类型、来源 turn、范围和路径，再追加到 MEMORY.md 和 memory_summary.md。scope 写入 YAML front matter。领域关联仅采用候选明确给出的已知工作区名称。
...
文件批次写入受 SQLite 写事务锁协调，临时文件替换单个文件；`.pending.json` 临时日志用于恢复中断的正文/索引批次，成功后删除。失败后的下一次记忆操作先恢复未完成批次。领域元数据可能已登记而正文未写完，只有文件批次成功才报告已保存。
...
测试覆盖完整分页、70% 预算与中文首尾截断、空结果、结构化结果、重复点击、超时、保存失败，以及临时目录中的 scope、来源校验、名称冲突、非 Git other、并发追加、重复写入和中断恢复。前端构建通过，设置接入后的完整 Vitest 670 项通过；存储实现的 Rust 154 项在上一轮通过，本轮未改 Rust。
