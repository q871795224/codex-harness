...
const CTE:&str="WITH selected AS (
 SELECT * FROM analysis_turns t WHERE (?3 IS NULL OR COALESCE(t.model,'')=?3) AND (?4 IS NULL OR t.thread_id=?4)
...
fn read_snapshot(
    ...
    let args = params![q.since, q.until, q.model, q.thread_id, q.kind, q.name];
    ...
    let models=tx.prepare(&format!("{CTE} SELECT model,COUNT(*),COUNT(DISTINCT thread_id),{SUM_TOKENS},SUM(rerouted) FROM activity GROUP BY model ORDER BY SUM(total_tokens) DESC,model"))?.query_map(args,|r|Ok(Model{model:r.get(0)?,turns:r.get(1)?,sessions:r.get(2)?,actual:tokens(r,3)?,rerouted_turns:r.get(9)?}))?.collect::<rusqlite::Result<Vec<_>>>()?;
    ...
    let daily=tx.prepare(&format!("{CTE}, days AS (
      ...
      SELECT strftime('%Y-%m-%d',at/1000,'unixepoch','localtime') date,COUNT(DISTINCT turn_id),{SUM_TOKENS} FROM days GROUP BY date ORDER BY date"))?.query_map(args,|r|Ok(Daily{date:r.get(0)?,turns:r.get(1)?,actual:tokens(r,2)?}))?.collect::<rusqlite::Result<Vec<_>>>()?;
    let hotspots=tx.prepare(&format!("{CTE} SELECT c.kind,c.name,COUNT(*),SUM(c.selected),SUM(c.reads),COUNT(DISTINCT t.thread_id),SUM(CASE WHEN c.status IN ('failed','declined','cancelled') THEN 1 ELSE 0 END),COALESCE(SUM(c.tokens),0),COALESCE(SUM(c.argument_tokens),0),COALESCE(SUM(c.result_tokens),0),SUM(CASE WHEN (c.kind='mcp' AND (c.argument_tokens IS NULL OR c.result_tokens IS NULL)) OR (c.kind!='mcp' AND c.tokens IS NULL) THEN 1 ELSE 0 END) FROM analysis_content c JOIN selected t USING(turn_id) WHERE c.at>=?1 AND c.at<?2 AND c.kind!='input' GROUP BY c.kind,c.name ORDER BY COUNT(*) DESC,c.name"))?.query_map(args,|r|Ok(Hotspot{kind:r.get(0)?,name:r.get(1)?,calls:r.get(2)?,selected:r.get(3)?,reads:r.get(4)?,sessions:r.get(5)?,failed:r.get(6)?,tokens:r.get(7)?,argument_tokens:r.get(8)?,result_tokens:r.get(9)?,missing_counts:r.get(10)?}))?.collect::<rusqlite::Result<Vec<_>>>()?;
    ...
    let sessions=tx.prepare(&format!("{CTE} SELECT thread_id,MAX(project),MIN(started_at),COUNT(*),{SUM_TOKENS},MAX(uncertain),MAX(rerouted) FROM activity GROUP BY thread_id ORDER BY {ordering},thread_id LIMIT 50 OFFSET {offset}"))?.query_map(args,|r|Ok(Session{thread_id:r.get(0)?,project:r.get(1)?,started_at:r.get(2)?,turns:r.get(3)?,actual:tokens(r,4)?,incomplete:r.get(10)?,rerouted:r.get(11)?}))?.collect::<rusqlite::Result<Vec<_>>>()?;
