  ...
  it('loads local state and normalizes stored preferences', async () => {
    const client = storage({
      ...
      [NAVIGATION_PREFERENCES_KEY]: JSON.stringify({
        ...
        sidebarListSplitRatio: 0.95, hiddenWorkspaceRoots: ['/old', '/old', 42, ''],
      }),
    ...
    expect(state.navigation).toMatchObject({
      ...
      hiddenWorkspaceRoots: ['/old'], pinnedThreadIds: ['thread-1'], sidebarWidth: 480, sidebarListSplitRatio: 0.8,
    })
