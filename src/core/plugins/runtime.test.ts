...
describe('plugin host lifecycle', () => {
  it('isolates activation failures and disposes effects in reverse order', async () => {
    ...
    const good = plugin('plugin-a', (ctx) => {
      ...
      ctx.slots.threadHeaderActions.register({ id: 'open', render: () => null })
    ...
    expect(host.resolvedThreadHeaderActions({ threadId: null, threadCwd: null, workspaceRoot: null })).toHaveLength(1)
    ...
    expect(host.resolvedThreadHeaderActions({ threadId: null, threadCwd: null, workspaceRoot: null })).toHaveLength(0)
