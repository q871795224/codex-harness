...
不要仅按来源目录判断归属。领域名称应简短、可读，优先复用已有名称；无法确定归属时跳过。
relatedWorkspaces 仅在领域记忆中填写证据支持的适用工作区名称，不把来源工作区自动当作关联。
...
export const MEMORY_OUTPUT_SCHEMA = {
  ...
  properties: { memories: { type: 'array', maxItems: 12, items: {
    ...
    required: ['title', 'kind', 'scope', 'content', 'applicability', 'evidence', 'sourceTurnIds', 'relatedWorkspaces'],
    properties: {
      ...
      relatedWorkspaces: { type: 'array', items: string },
    },
  } } },
}
...
export function extractionPrompt(transcript: string, catalog: MemoryCatalog, effectiveWindow: number, instructions = MEMORY_INSTRUCTIONS, budgetPercent = 70) {
  const prefix = `可用范围：${JSON.stringify(catalog)}\n以下是当前会话的历史记录（JSONL，超长时中间会省略）：\n`
